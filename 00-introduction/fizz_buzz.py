...
def fizz_buzz(i, prediction):
    return [i, "fizz", "buzz", "fzbz"][prediction]


def fizz_buzz_simple(i):
        if   i % 15 == 0: return 'fzbz'
        elif i % 5  == 0: return 'fizz'
        elif i % 3  == 0: return 'buzz'
        else:             return i
...
with tf.Session() as sess:
    ...
    # for reference
    truth = [fizz_buzz_simple(i) for i in range(1, 101)]
    ...
    output = map(fizz_buzz, numbers, teY)

    for z in zip(numbers, truth, output):
        print("%d\t%s\t%s\t%s" % (z[0], z[1], z[2], '' if z[1] == z[2] else ' x'))
